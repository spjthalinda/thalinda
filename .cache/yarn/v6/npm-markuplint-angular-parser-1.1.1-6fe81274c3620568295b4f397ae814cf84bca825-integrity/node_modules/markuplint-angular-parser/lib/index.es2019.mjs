import { flattenNodes, parseRawTag, attrTokenizer } from '@markuplint/html-parser';
import { uuid } from '@markuplint/parser-utils';
import * as ngHtmlParser from 'angular-html-parser';
import { visitAll } from 'angular-html-parser/lib/compiler/src/ml_parser/ast';

const getSourceSpan = (nodeOrSourceSpan) => "sourceSpan" in nodeOrSourceSpan ? nodeOrSourceSpan.sourceSpan : nodeOrSourceSpan;
...
const visitor = {
  visitElement({
    ...
    namespace = ((_a = attrs.find((attr) => attr.name === "xmlns")) == null ? void 0 : _a.value) || (nodeName === "svg" || nodeName.startsWith("svg:") ? "http://www.w3.org/2000/svg" : namespace || "http://www.w3.org/1999/xhtml");
    const { endSpace, selfClosingSolidus } = parseRawTag(startTagText, partialStartTag.startLine, partialStartTag.startCol, partialStartTag.startOffset);
  ...
  visitAttribute(attribute, { nodeList, ...options }) {
    ...
    const node = attrTokenizer(getRaw(attribute, options.text), start.line + 1, start.col, start.offset);
    const _value = value.trim();
    const dynamicName = /^[#*]/.test(name) || /^\[[^.[\]]+]$/.test(name) || /^\([^().]+\)$/.test(name);
    const dynamicValue = /^{{.*}}$/.test(_value);
    if (dynamicName || dynamicValue) {
      node.isDynamicValue = true;
    }
    const potentialName = name.replace(/^\[attr\./, "").replace(/[()*@[\]]/g, "");
  ...
  visitDocType(docType, { nodeList, ...options }) {
    ...
    const node = {
      ...
      nodeName: "#doctype",
      publicId: (_a = matched == null ? void 0 : matched[2]) != null ? _a : "",
      systemId: (_b = matched == null ? void 0 : matched[5]) != null ? _b : ""
    };
    nodeList.push(node);
  },
...
const parse = (text) => {
  ...
  const document = {
    ...
    isFragment: !nodeList.some((node) => node.type === "doctype" || node.type === "starttag" && node.nodeName.toLowerCase() === "html")
  };
  if (errors.length > 0) {
